...
export function LoginForm({ locale }: LoginFormProps) {
  ...
  const [enabledProviders, setEnabledProviders] = useState<string[]>(['google', 'discord', 'credentials']);

  useEffect(() => {
    loadTranslations(locale, 'common').then(setTranslations);
    // Aktív OAuth provider-ek lekérése
    fetch('/api/admin/settings/oauth')
      .then((res) => res.json())
      .then((data) => {
        if (data.enabledProviders && data.enabledProviders.length > 0) {
  ...
  const hasOAuthProviders = enabledProviders.some((p) => p === 'google' || p === 'discord');
    ...
    <div className="space-y-6">
      ...
      {hasOAuthProviders && (
        <>
          ...
          {/* Divider - csak ha van OAuth és Credentials is */}
          {hasCredentials && (
      ...
      {/* Email/Password Form - csak ha credentials engedélyezve */}
      {hasCredentials && (
