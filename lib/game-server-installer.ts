...
async function installARKSharedFiles(
  ...
  const installScript = `
    ...
    chown root:root ${sharedPath}
    cd ${sharedPath}
    
    # Globális SteamCMD használata
    # Hivatalos SteamCMD dokumentáció: https://developer.valvesoftware.com/wiki/SteamCMD
    ...
    # Fontos: force_install_dir MINDIG a login előtt kell használni!
    echo "Installing ARK server files (this may take a while)..."
    echo "SteamCMD App ID: ${steamAppId}"
    echo "Install directory: ${sharedPath}"
    ...
    while [ $RETRY_COUNT -lt $MAX_RETRIES ]; do
      ...
      HOME="$STEAM_HOME" $STEAMCMD +force_install_dir ${sharedPath} +login anonymous +app_update ${steamAppId} validate +quit
      EXIT_CODE=$?
