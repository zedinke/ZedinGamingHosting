...
# Left 4 Dead szerver telepítése globális SteamCMD-vel
echo "Installing Left 4 Dead dedicated server..."
...
while [ $RETRY_COUNT -lt $MAX_RETRIES ]; do
  ...
  if [ -d "$SERVER_DIR/left4dead" ] || [ -d "$SERVER_DIR/steamapps/common/Left 4 Dead Dedicated Server" ]; then
...
mkdir -p "$SERVER_DIR/left4dead/cfg"
chown -R root:root "$SERVER_DIR"
chmod -R 755 "$SERVER_DIR"
`;
