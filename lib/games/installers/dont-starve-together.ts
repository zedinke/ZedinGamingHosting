/**
 * Don't Starve Together telepítő script
 */
...
# Don't Starve Together szerver telepítése globális SteamCMD-vel
...
while [ $RETRY_COUNT -lt $MAX_RETRIES ]; do
  ...
  if [ -f "$SERVER_DIR/bin/dontstarve_dedicated_server_nullrenderer" ] || [ -d "$SERVER_DIR/steamapps/common/Don't Starve Together Dedicated Server" ]; then
...
mkdir -p "$SERVER_DIR/DoNotStarveTogether"
chown -R root:root "$SERVER_DIR"
chmod -R 755 "$SERVER_DIR"
`;