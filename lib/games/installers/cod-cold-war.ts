/**
 * Call of Duty: Black Ops Cold War telepítő script
 */
...
# Black Ops Cold War szerver telepítése
echo "Installing Call of Duty: Black Ops Cold War dedicated server..."
...
while [ $RETRY_COUNT -lt $MAX_RETRIES ]; do
  ...
  HOME="$STEAM_HOME" /opt/steamcmd/steamcmd.sh +force_install_dir "$SERVER_DIR" +login anonymous +app_update 1357840 validate +quit
  EXIT_CODE=$?
  ...
  if [ -d "$SERVER_DIR" ] && [ -f "$SERVER_DIR/coldwar_server.exe" ]; then
