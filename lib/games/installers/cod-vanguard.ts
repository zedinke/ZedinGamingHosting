/**
 * Call of Duty: Vanguard telepítő script
 */
...
# Vanguard szerver telepítése
echo "Installing Call of Duty: Vanguard dedicated server..."
...
while [ $RETRY_COUNT -lt $MAX_RETRIES ]; do
  ...
  HOME="$STEAM_HOME" /opt/steamcmd/steamcmd.sh +force_install_dir "$SERVER_DIR" +login anonymous +app_update 1687720 validate +quit
  EXIT_CODE=$?
  ...
  if [ -d "$SERVER_DIR" ] && [ -f "$SERVER_DIR/vanguard_server.exe" ]; then
