...
# DayZ szerver telepítése globális SteamCMD-vel
...
while [ $RETRY_COUNT -lt $MAX_RETRIES ]; do
  ...
  HOME="$STEAM_HOME" /opt/steamcmd/steamcmd.sh +force_install_dir "$SERVER_DIR" +login anonymous +app_update 221100 validate +quit
  EXIT_CODE=$?
  ...
  if [ -f "$SERVER_DIR/DayZServer" ] || [ -d "$SERVER_DIR/steamapps/common/DayZ Server" ]; then
