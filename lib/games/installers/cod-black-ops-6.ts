/**
 * Call of Duty: Black Ops 6 telepítő script
 */
...
# Black Ops 6 szerver telepítése
echo "Installing Call of Duty: Black Ops 6 dedicated server..."
...
while [ $RETRY_COUNT -lt $MAX_RETRIES ]; do
  ...
  HOME="$STEAM_HOME" /opt/steamcmd/steamcmd.sh +force_install_dir "$SERVER_DIR" +login anonymous +app_update 2084520 validate +quit
  EXIT_CODE=$?
  ...
  if [ -d "$SERVER_DIR" ] && [ -f "$SERVER_DIR/bo6_server.exe" ]; then
