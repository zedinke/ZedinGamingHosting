/**
 * Call of Duty: Modern Warfare 2024 telepítő script
 */
...
# Modern Warfare 2024 szerver telepítése
echo "Installing Call of Duty: Modern Warfare 2024 dedicated server..."
...
while [ $RETRY_COUNT -lt $MAX_RETRIES ]; do
  ...
  HOME="$STEAM_HOME" /opt/steamcmd/steamcmd.sh +force_install_dir "$SERVER_DIR" +login anonymous +app_update 2149880 validate +quit
  EXIT_CODE=$?
  ...
  if [ -d "$SERVER_DIR" ] && [ -f "$SERVER_DIR/cod_server.exe" ]; then
