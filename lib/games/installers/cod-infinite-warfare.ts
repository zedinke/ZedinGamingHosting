/**
 * Call of Duty: Infinite Warfare telepítő script
 */
...
# Infinite Warfare szerver telepítése
echo "Installing Call of Duty: Infinite Warfare dedicated server..."
...
while [ $RETRY_COUNT -lt $MAX_RETRIES ]; do
  ...
  HOME="$STEAM_HOME" /opt/steamcmd/steamcmd.sh +force_install_dir "$SERVER_DIR" +login anonymous +app_update 292730 validate +quit
  EXIT_CODE=$?
  ...
  if [ -d "$SERVER_DIR" ] && [ -f "$SERVER_DIR/iw_server.exe" ]; then
