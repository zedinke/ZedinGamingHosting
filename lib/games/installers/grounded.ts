...
# Grounded szerver telepítése globális SteamCMD-vel
...
while [ $RETRY_COUNT -lt $MAX_RETRIES ]; do
  ...
  HOME="$STEAM_HOME" /opt/steamcmd/steamcmd.sh +force_install_dir "$SERVER_DIR" +login anonymous +app_update 1422210 validate +quit
  EXIT_CODE=$?
  ...
  if [ -f "$SERVER_DIR/GroundedServer" ] || [ -d "$SERVER_DIR/steamapps/common/Grounded Dedicated Server" ]; then
...
if [ -f "$SERVER_DIR/GroundedServer" ]; then
  chmod +x "$SERVER_DIR/GroundedServer"
fi
`;