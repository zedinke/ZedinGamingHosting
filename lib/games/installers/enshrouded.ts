...
# Enshrouded szerver telepítése globális SteamCMD-vel
...
while [ $RETRY_COUNT -lt $MAX_RETRIES ]; do
  ...
  HOME="$STEAM_HOME" /opt/steamcmd/steamcmd.sh +force_install_dir "$SERVER_DIR" +login anonymous +app_update 2915120 validate +quit
  EXIT_CODE=$?
  ...
  if [ -f "$SERVER_DIR/enshrouded_server" ] || [ -d "$SERVER_DIR/steamapps/common/Enshrouded Dedicated Server" ]; then
...
if [ -f "$SERVER_DIR/enshrouded_server" ]; then
  chmod +x "$SERVER_DIR/enshrouded_server"
fi
`;