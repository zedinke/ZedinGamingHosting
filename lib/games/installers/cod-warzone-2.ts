/**
 * Call of Duty: Warzone 2.0 telepítő script
 */
...
# Warzone 2.0 szerver telepítése
echo "Installing Call of Duty: Warzone 2.0 dedicated server..."
...
while [ $RETRY_COUNT -lt $MAX_RETRIES ]; do
  ...
  HOME="$STEAM_HOME" /opt/steamcmd/steamcmd.sh +force_install_dir "$SERVER_DIR" +login anonymous +app_update 1958861 validate +quit
  EXIT_CODE=$?
  ...
  if [ -d "$SERVER_DIR" ] && [ -f "$SERVER_DIR/warzone_server.exe" ]; then
