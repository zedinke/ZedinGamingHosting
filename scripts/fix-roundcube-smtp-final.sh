...
# Roundcube SMTP config teljes javítása (port 25, auth nélkül)

cd /usr/share/roundcube/config

# Biztonsági mentés
cp config.inc.php config.inc.php.bak.$(date +%Y%m%d_%H%M%S)
...
echo "✅ Config teljesen újraírva (SMTP port 25, auth nélkül)"
